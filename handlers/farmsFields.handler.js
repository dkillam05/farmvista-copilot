...
// Rev: 2026-01-02-handler-fields-only
//
// Handles farms + fields only.
...
function extractFarmNameGuess(q){
  // Very simple heuristic: look for "on <farm>" or "in <farm>"
  // Example: "list fields on lov shack"
  const m = q.match(/\b(?:on|in)\s+([a-z0-9][a-z0-9\s\-\._]{1,60})$/i);
  if (!m) return '';
...
export async function handleFarmsFields({ question, snapshot, user, includeArchived = false }){
  ...
  if (!cols.ok){
    return { ok: false, answer: 'Snapshot not loaded (missing farms/fields).', meta:{ reason: cols.reason } };
  }
  ...
  // Try resolving using tryResolveField which can return candidates.
  ...
  if (res.ok && res.resolved && res.fieldId){
    ...
    if (!b.ok){
      return { ok: true, answer: 'Found the field id but could not load details.', meta:{ routed:'farmsFields', intent:'field_lookup', reason:b.reason } };
    }
    ...
    parts.push(`Status: ${status || 'active'}`);
  ...
  if (res.ok && res.resolved === false && Array.isArray(res.candidates) && res.candidates.length){
    const lines = res.candidates.map(c => `• ${formatFieldOptionLine({ snapshot, fieldId: c.fieldId })}`).join('\n');
    return {
      ok: true,
